...
export const createTimetableEntry = async (req, res) => {
    try {
        const { day, period, startTime, endTime, room, color, isBreak, breakLabel, sectionId, subjectId, teacherId } = req.body;
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            for (const entry of entries) {
                const { day, period, startTime, endTime, room, color, isBreak, breakLabel, sectionId, subjectId, teacherId } = entry;
                const formattedDay = day.toUpperCase();
                const periodNumber = Number(period);
                const treatAsBreak = Boolean(isBreak);
...
export const getWeeklyTimetableBySection = async (req, res) => {
    try {
...
export const getTeacherMySubjectTimetable = async (req, res) => {
    try {
        const userId = req.user.id;
        const { day } = req.query;
        if (!day || typeof day !== "string") {
        ...
        const merged = [...normalizedPeriods, ...normalizedBreaks].sort((a, b) => a._sortTime.localeCompare(b._sortTime));
...
export const getTeacherMySubjectWeekly = async (req, res) => {
    try {
        const userId = req.user.id;
...
export const getDailyTimetableBySection = async (req, res) => {
    try {
        const { sectionId } = req.params;
        const { day } = req.query;
        if (!sectionId) {
