...
export const markBulkAttendance = async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(students.map((s) => prisma.attendance.upsert({
            where: {
                studentId_date: {
...
export const updateStudentYearlyAttendance = async (req, res) => {
    try {
        ...
        const result = await prisma.$transaction(attendanceRecords.map((record) => {
...
export const getStudentAttendancePercentage = async (req, res) => {
    try {
...
export const getStudentMonthlyTrends = async (req, res) => {
    try {
        const { studentId } = req.params;
        const { year } = req.query;
        if (!studentId) {
...
//
export const getStudentWeeklyTrends = async (req, res) => {
    try {
        const { studentId } = req.params;
        ...
        const targetYear = year ? parseInt(year, 10) : currentSystemDate.getFullYear();
        const targetMonth = month ? parseInt(month, 10) : (currentSystemDate.getMonth() + 1); // 1-12
...
export const getStudentHeatmapGrid = async (req, res) => {
    try {
        const { studentId } = req.params;
        const { year } = req.query;
        if (!studentId) {
