 ...
 */
export const createAssessmentComponents = async (req, res) => {
    try {
        ...
        const created = await prisma.$transaction(components.map((c) => prisma.assessmentComponent.create({
...
export const createGradingScale = async (req, res) => {
    try {
        ...
        const newScales = await prisma.$transaction(scales.map((s) => prisma.gradingScale.create({ data: s })));
    ...
    try {
        ...
        const students = await prisma.student.findMany({
            ...
            select: {
                ...
                rollNumber: true,
                marks: {
                    where: { assessmentComponentId: assessmentComponentId },
 ...
 */
export const getStudentFullSummary = async (req, res) => {
    try {
