...
export const admitStudent = async (req, res) => {
    const { email, password, firstName, lastName, dateOfBirth, gender, address, city, state, bloodGroup, profileImage, phoneNumber, sectionId, admissionNumber, rollNumber } = req.body;
    try {
...
export const getAllStudents = async (req, res) => {
    try {
        const { search, sectionId, classId, gender, status, page = "1", limit = "6", } = req.query;
        ...
        if (sectionId) {
            const isUUID = /^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$/.test(String(sectionId));
            if (isUUID) {
...
//--bulk admission---
export const bulkAdmitStudents = async (req, res) => {
    if (!req.file)
