...
export const createTimetableEntry = async (req: Request, res: Response) => {
  try {
    ...
    if (!treatAsBreak && targetTeacherId) {
      ...
      if (teacherConflict) {
        return res.status(400).json({
          success: false,
          message:
            "Teacher is already assigned to another section at this time.",
        });
      }
    }
    ...
    if (sectionConflict) {
      return res.status(400).json({
        success: false,
        message:
          "This section already has a scheduled block for this period.",
      });
    }
    ...
    const entry = await prisma.timetable.create({
      data: {
        ...
        teacherId: treatAsBreak ? null : targetTeacherId,
      },
      include: {
        teacher: true,
        subject: true,
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const entry of entries) {
        ...
        if (!treatAsBreak && teacherId) {
          ...
          if (teacherConflict) {
            throw new Error(
              `Conflict: Teacher is already assigned at ${formattedDay}, Period ${periodNumber}.`
            );
          }
        }
        ...
        if (sectionConflict) {
          throw new Error(
            `Conflict: Section already has a class scheduled at ${formattedDay}, Period ${periodNumber}.`
          );
        }
  ...
  try {
    ...
    if (filter === "mySubject") {
      ...
      const matchingSubjects = await prisma.subject.findMany({
        where: {
          name: {
            in: assignedSubjectNames as string[],
          },
        },
        select: {
          id: true,
      ...
      whereClause.subjectId = {
        in: matchingSubjects.map((s) => s.id),
      };
    }
    ...
    const formattedSchedule = teacherPeriods.map((row) => ({
      ...
      endTime: row.endTime,
      period: row.period,
      isActive: isCurrentPeriodActive(
        row.startTime,
        row.endTime,
        day
      ),
...
export const getWeeklyTeacherTimetable = async (
  req: Request,
  res: Response
) => {
  try {
...
export const updateTimetableEntry = async (req: Request, res: Response) => {
  try {
    ...
    const treatAsBreak =
      isBreak !== undefined ? Boolean(isBreak) : existingEntry.isBreak;
    ...
    if (!treatAsBreak) {
      if (teacherId) {
        ...
        // IMPORTANT: store Teacher.id, NOT User.id
        targetTeacherId = teacherProfile.id;
      } else {
        targetTeacherId = existingEntry.teacherId;
      }
    }
    ...
    const updatedEntry = await prisma.timetable.update({
      ...
      data: {
        startTime:
          startTime !== undefined ? startTime : existingEntry.startTime,

        endTime:
          endTime !== undefined ? endTime : existingEntry.endTime,

        room:
          room !== undefined ? room || null : existingEntry.room,

        color:
          color !== undefined ? color || null : existingEntry.color,

        isBreak: treatAsBreak,

        breakLabel: treatAsBreak
          ? breakLabel || "Recess"
          : null,

        subjectId: treatAsBreak
          ? null
          : subjectId !== undefined
          ? subjectId
          : existingEntry.subjectId,

        teacherId: treatAsBreak
          ? null
          : targetTeacherId,
      },
      include: {
