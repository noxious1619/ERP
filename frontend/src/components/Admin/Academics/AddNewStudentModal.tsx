...
export default function AddNewStudentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!firstName || !lastName || !email || !admissionNumber || !dateOfBirth || !gender || !sectionId) {
      setErrorMsg("Please fill in all required fields (First/Last Name, Email, Admission No, DOB, Gender, and Section).")
      return
    }
    ...
    try {
      ...
      if (isEdit) {
        const res = await axios.patch(`${API_BASE}/api/students/${studentToEdit.id}`, payload, { headers })
        if (res.data.success) {
          onSuccess()
        } else {
        ...
        const res = await axios.post(`${API_BASE}/api/students/admit`, payload, { headers })
        if (res.data.success) {
          onSuccess()
        } else {
      ...
      setErrorMsg(backendError ? `${backendMessage}: ${backendError}` : (backendMessage || "An error occurred during submission."))
    } finally {
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-white shadow-xl my-6 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {/* Row 3: Admission & Roll No */}
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Admission Number <span className="text-red-500">*</span></label>
                <input
                  type="text"
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Roll Number</label>
                <input
                  type="text"
                  value={rollNumber}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Class <span className="text-red-500">*</span></label>
                <select
                  value={classId}
                  onChange={(e) => {
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Section <span className="text-red-500">*</span></label>
                <select
                  value={sectionId}
