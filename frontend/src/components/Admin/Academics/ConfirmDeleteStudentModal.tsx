...
export default function ConfirmDeleteStudentModal({
  isOpen,
  onClose,
  onConfirm,
  isDeleting = false,
    ...
    >
      <div className="bg-white rounded-2xl border border-gray-100 max-w-md w-full shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 pb-6 flex flex-col items-center text-center">
          ...
          <h3 className="font-bold text-gray-900 text-lg leading-snug">Delete Students</h3>
          <p className="text-sm text-gray-500 mt-2 leading-relaxed">
            Are you sure you want to delete <span className="font-semibold text-gray-800">{count} {count === 1 ? "selected student" : "selected students"}</span>? This action is permanent, will delete all associated student data (including attendance, grades, fee records), and cannot be undone.
          </p>
        </div>
        ...
        <div className="border-t border-gray-100 px-6 py-4 bg-gray-50 flex items-center gap-3">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isDeleting}
            className="flex-1 bg-[#4285F4] hover:bg-blue-600 disabled:opacity-60 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition shadow-sm cursor-pointer flex items-center justify-center gap-2"
          >
            {isDeleting ? (
              <>
