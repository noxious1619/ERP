...
export default function StudentTable({
  ...
  const allSelectedOnPage = students.length > 0 && students.every((s) => selectedIds.includes(s.id))
      ...
      <table className="w-full min-w-[900px] text-sm">
        <thead>
          <tr className="border-b border-gray-100">
            ...
            {HEADERS.map((h) => (
              <th key={h} className="px-4 py-4 text-left font-semibold text-gray-900 sticky top-0 bg-gray-50 z-10 border-b border-gray-200">
                {h}
              </th>
            ))}
        ...
        <tbody>
          ...
          {!isLoading && error && (
            <tr>
              <td colSpan={10} className="px-6 py-12 text-center text-sm text-red-500 font-medium">
                {error}
              </td>
            </tr>
          )}
          ...
          {!isLoading && !error && students.length === 0 && (
            <tr>
              <td colSpan={10} className="px-6 py-12 text-center text-sm text-gray-400 font-medium">
          ...
          {/* ── data rows ── */}
          {!isLoading && !error && students.map((student, idx) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-5 text-gray-700">{student.admissionNumber}</td>
                <td className="px-4 py-5 font-semibold text-gray-900">{fullName}</td>
                <td className="px-4 py-5 text-gray-600">{className}</td>
                <td className="px-4 py-5 text-gray-600">{sectionName}</td>
                <td className="px-4 py-5 text-gray-400 italic font-medium">N/A</td>
                <td className="px-4 py-5 text-gray-600">{contact}</td>
                <td className="px-4 py-5">
                  <StatusBadge status={student.isActive !== false ? "Active" : "Inactive"} />
                </td>
                <td className="px-4 py-5 text-gray-400 italic font-medium">N/A</td>
