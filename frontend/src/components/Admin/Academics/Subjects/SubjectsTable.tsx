...
function TypeBadge({ type }: { type: SubjectTypeVal }) {
  ...
  return (
    <span
      className={`rounded-full px-3 py-1 text-xs font-semibold ${styles[type] || "bg-gray-100 text-gray-700"}`}
    >
      {type}
    </span>
  );
...
export default function SubjectsTable({
  ...
  const allSelectedOnPage =
    subjects.length > 0 && subjects.every((s) => selectedIds.includes(s.id));
      ...
      <table className="w-full min-w-[900px] text-sm">
        <thead>
          <tr className="border-b border-gray-100">
            ...
            <th className="px-4 py-4 text-left font-semibold text-gray-900 sticky top-0 bg-gray-50 z-10 border-b border-gray-200">
              Sub Name
            </th>
            <th className="px-4 py-4 text-left font-semibold text-gray-900 sticky top-0 bg-gray-50 z-10 border-b border-gray-200">
              Sub Code
                ...
                >
                  <td className="px-6 py-5">
                    <input
                      ...
                      checked={isChecked}
                      onChange={(e) =>
                        onSelectRow(subject.id, e.target.checked)
                      }
                      className="h-4 w-4 rounded border-gray-300 accent-blue-600 cursor-pointer"
                    />
                  </td>
                  ...
                  <td className="px-4 py-5 text-gray-500 font-mono text-xs uppercase tracking-wider">
                    {subject.code}
                  </td>
