...
export default function TeacherHeader({
  totalCount, search, onSearchChange, onAddClick, onExportCSV
}: TeacherHeaderProps) {
  return (
    <div className="flex items-center justify-between">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">Teachers</h1>
        <p className="mt-0.5 text-sm text-gray-500">{totalCount} teachers found</p>
      </div>
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={onExportCSV}
          className="flex h-10 items-center gap-1.5 rounded-full border border-blue-600 px-5 text-sm font-medium text-blue-600 transition-colors hover:bg-blue-50 bg-white"
        >
          Export CSV
        </button>
      </div>
