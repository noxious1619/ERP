      ...
      <div className="relative mt-6 flex flex-col gap-6">
        ...
        {isLoading ? (
          ...
          classFilteredItems.map((item) => {
            const isActive = isToday && isPeriodActive(item.startTime, item.endTime);
            return (
              <div
                key={item.id}
                className={`relative z-10 flex py-2 items-center rounded-[14px] border bg-white px-4 shadow-[0px_2px_6px_rgba(0,0,0,0.03)] ${
                  isActive ? "border-[#3A72FF] ring-2 ring-[#3B4FE8]/20 border-l-4 border-l-[#3A72FF]" : "border-[#D7DAE4]"
                }`}
              >
                ...
                <div className="flex flex-col items-end justify-center">
                  <span className={`text-[11px] font-medium tracking-[1px] ${isActive ? "text-[#3B4FE8]" : "text-[#8A8A8A]"}`}>
                    {item.startTime}
                  </span>
